...
exports.askMrEagles = void 0;
const https_1 = require("firebase-functions/v2/https");
const firestore_1 = require("firebase-admin/firestore");
const db = (0, firestore_1.getFirestore)();
exports.askMrEagles = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'User must be authenticated to consult Mr. EaglEs.');
    }
    const tenantId = String(request.auth.token.tenantId ?? 'eagles');
    const prompt = request.data?.prompt ?? '';
    if (!prompt || prompt.trim().length === 0) {
        throw new https_1.HttpsError('invalid-argument', 'A prompt query is required.');
    }
